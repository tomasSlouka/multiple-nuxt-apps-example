            ...
            <form @submit.prevent.once='submitForm()' class='grid gap-20' autocomplete="off">
                ...
                <div class='grid gap-20'>
                    ...
                    <div>
                        ...
                        <p class='small'>{{charCounter}} / 255</p>
                    </div>
                    <div>
                ...
                <div>
                    ...
                    <nuxt-link tag='button' class='cta orange white justify-self-start' to='/admin/newsletter/messages'>Cancel</nuxt-link>
                </div>
            </form>
        </div>
    </div>
    ...
    methods: {
        ...
        async submitForm() {
            await this.$axios.$post('/message', {
                ...
                'newsletter_id': this.$store.state.auth.userData.newsletter_id,
            })
            .then((response) => {
                console.log(response)
                this.$router.push('/admin/newsletter/messages')
            }, (error) => {
                console.log(error)
            });
        }
